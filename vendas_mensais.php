...
foreach ($vendas as $venda) {
    ...
    foreach ($itens as $item) {
        ...
        $linha[] = "$nome ({$item['quantidade']})";
    }
    $vendaItens[$venda['venda_id']] = implode(', ', $linha);
}
...
<?php if (count($vendas) > 0): ?>
    ...
    <div class="total-final">
        Total do Mês: <?= number_format($total_geral, 2) ?> MT
    </div>
<?php else: ?>
    <p style="text-align: center; color: #6b7280;">Nenhuma venda foi realizada este mês.</p>
<?php endif; ?>
